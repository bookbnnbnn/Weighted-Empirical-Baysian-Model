...
class QEB:
    def __init__(
        ...
        self.X_k = np.array([]) if grid_points is None else - \
            1 / 2 * grid_points**2
    ...
    def create_data(self) -> np.ndarray:
        ...
        for j in range(self.types_of_proteins):
            ...
            for i in range(self.numbers_of_types):
                ...
                A_ij_tilde = -3 / 2 * \
                    np.log(2 * np.pi * 1 / beta) + np.log(A_ij)
    ...
    def caculate_constants(self) -> Tuple[np.ndarray]:
        ...
        for j in range(self.types_of_proteins):
            ...
            for i in range(self.numbers_of_each_type[j]):
                cov = np.cov(self.X_k, self.data_log_list[j][i])[0, 1]
                var = np.var(self.X_k)
                beta_ols = cov / var
                data = self.data_log_list[j][i]
                A_ij = data.mean() - beta_ols * X_k_distance
                estimated_A_ij_tilde_list.append(A_ij)
                variance = self.variances_of_error[j][i]
                # Sum `k`
                B_j_list.append(sum((data - A_ij)**2 / (2 * variance)))
                C_j_list.append(
                    sum((data - A_ij) * self.grid_points**2 / (2 * variance)))
                D_j_list.append(sum(self.grid_points**4 / (8 * variance)))
                sigma_j_bar *= variance
            self.estimated_A_ij_tilde.append(estimated_A_ij_tilde_list)
            # Sum `i`
    ...
    def negative_log_likelihood(self, params: Tuple) -> float:
        ...
        return -sum(-1 / 2 * np.log(self.sigma_j_bar * E_j) - (sigma_0 * (4 * self.B_j * self.D_j -
                    self.C_j**2) - 2 * mu_0 * self.C_j - 2 * self.B_j - 2 * mu_0**2 * self.D_j) / (2 * E_j))
    ...
    def estimate_beta_1j_by_Bayse(self) -> np.ndarray:
        ...
        self.beta_1j_hat = (
            self.mu_0_hat + self.sigma_0_hat * self.C_j) / self.E_j
