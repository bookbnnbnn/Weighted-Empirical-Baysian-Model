...
def caculcate_bs_tilde_single(
    ...
    for weight_matrix, X_tilde, y_tilde, mu_tilde, lambda_ in zip(weight_matrix_all, X_tilde_all, y_tilde_all, mu_tilde_all, lambda_all):
        b = v * s / 2
        first = (y_tilde - X_tilde @ mu_tilde).T @ weight_matrix @ (y_tilde - X_tilde @ mu_tilde)
        second = lambda_ * (mu_tilde - mu).T @ sigma_matrix @ (mu_tilde - mu)
        b_tilde.append(b + 1 / 2 * (first + second))
    bs_tilde.append(np.array(b_tilde))
    return {name: np.array(bs_tilde)}
...
def caculate_tilde_single(
    ...
    return mu_tilde_all, sigma_matrixs_tilde, as_tilde, bs_tilde
...
def caculate_tilde(
    ...
    for y_tilde_all, lambda_all, weight_matrix_all, sigma_matrix, v, s, mu, X_tilde_all, name in \
        ...
        mus_tilde_all, sigma_matrixs_tilde_all, as_tilde_all, bs_tilde_all = caculate_tilde_single(
    ...
    return mus_tilde_dict, sigma_matrixs_tilde_dict, as_tilde_dict, bs_tilde_dict
...
def negative_log_likelihood(
    ...
    ans = 0
    X_tilde_all, y_tilde_all, lambda_all, weight_matrix_all =\
    Xs_tilde[name], ys_tilde[name], lambdas[name], weight_matrixs[name]
    mu_tilde_all = caculate_mus_tilde_single(X_tilde_all, y_tilde_all, weight_matrix_all, lambda_all, sigma_matrix, mu, name)[name]
...
def estimator_negative_log_likelihood( 
    ...
    return negative_log_likelihood(v, s, mu, sigma_matrix, Xs_tilde, ys_tilde, \
                                   lambdas, weight_matrixs, name)
...
def caculate_weight_matrix(X_tilde_all, y_tilde_all, betas_em, sigmas_em, alpha):
    ...
    for idx, elements in enumerate(zip(X_tilde_all, y_tilde_all, betas_em, sigmas_em)):
        ...
        # print(sigma_em)
        weights[idx] = np.diag(coeff * exponential / len(X_tilde))
        print("weight")
