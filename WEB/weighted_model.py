...
class WEB:
    ...
    def create_data(self,
        ...
        contaminated_beta0_btw_group: float
            Contaminated beta0 value between groups.
        contaminated_beta1_btw_group: float
            Contaminated beta1 value between groups.
        contaminated_beta0_within_group: float, optional
            Contaminated beta0 value within groups (default is None).
        contaminated_beta1_within_group: float, optional
            Contaminated beta1 value within groups (default is None).
        ...
        X_tilde = [[np.concatenate((np.ones((grid_num, 1)), (-1 / 2 * distances_to_center ** 2).reshape(-1, 1)),
                                axis=1).tolist()] * in_group_num]
        self.distances_to_center = {group_name[idx]: val for idx, val in
                                    enumerate(np.array([[distances_to_center] * in_group_num] * group_num))}
        ...
        self.weights = {group_name[idx]: val for idx, val in
                        enumerate([np.ones((in_group_num, grid_num))] * group_num)}
        ...
        for mu, sigma_all, lambda_all in \
            ...
            for _ in lambda_all:
                if counts > 0:
                    beta.append(
                        multivariate_normal.rvs([contaminated_beta0_btw_group, contaminated_beta1_btw_group],
        ...
        for beta_all, sigma_all, weight_all, X_tilde_all in \
            ...
            for beta, sigma, weight, X_tilde in zip(beta_all, sigma_all, weight_all, X_tilde_all):
                ...
                if contained_ratio_data > 0:
                    ...
                    data[index] = multivariate_normal.rvs(X_tilde[index, :] @ np.array(contaminated_beta),
                                                        sigma * np.diag(weight[index]))
                else:
                    index = []
                y_tilde.append(data)
    ...
    def read_data(
        ...
        mode: str
            Mode for processing residue names ("all", "three", or "separate"). Defaults to "separate".

        Returns
        ----------
        Dict[str, np.ndarray]: 
            Dictionary containing the log values of the interpolated data for each atom type.
        """
        # Read map file
        data, grid_size, origin = read_map(root_map)

        # Read pdb file and process data
        ...
        # Process residue names based on the specified mode
        if mode == "all":
        ...
        # Unique residue names and atom coordinates
        residue_names_unique = df_processed["residue_name"].unique()
        self.atoms_order = {name: df_processed.groupby(["residue_name"]).get_group(name)["atom_name"].values for name in
                            residue_names_unique}
        atom_points = np.column_stack((df_processed.x_coord, df_processed.y_coord, df_processed.z_coord))

        # Generate grid points and interpolate data
    ...
    def parameters_initial(self, 
        ...
        self.mus_initial = {}  # Dictionary to store initial values of mus
        self.sigmas_initial = {}  # Dictionary to store initial values of sigmas
        self.betas_initial = {}  # Dictionary to store initial values of betas
        self.betas_OLS = {}  # Dictionary to store OLS estimates of betas
        self.weights = {}  # Dictionary to store weights
        self.lambdas = {}  # Dictionary to store lambdas

        for name in self.data_log:
            beta_OLS = []

            # Calculate OLS estimates of betas for each data point
            ...
            # Initialize betas and sigmas for each data point
            for X_tilde, y_tilde in zip(self.Xs_tilde[name], self.data_log[name]):
    ...
    def empirical_bayes(self):
        """
        Perform empirical Bayes estimation for the model parameters.

        Updates self.betas_EB based on the empirical Bayes approach.

        Returns
        ----------
        self.betas_EB: dict
            Dictionary containing the empirical Bayes estimated beta values for each group.
        """
        # Initialize self.betas_EB with initial beta values
        self.betas_EB = self.betas_initial.copy()

        # Iterate over each group in the data
        for name in self.data_log:
            # Initialize arrays for lambda, sigma, and weights
            ...
            # Calculate mean of OLS beta values
            mu = np.mean(self.betas_OLS[name], axis=0)

            # Calculate sigma matrix using the MDPDE method
            sigma_matrix = calculate_sigma_matrix_MDPDE(self.betas_OLS[name], mu, lambda_ones, 1)

            # Update beta values using the WEB method
            self.betas_EB[name] = calculate_betas_WEB(self.Xs_tilde[name], self.data_log[name],
                                                        weight_ones, lambda_ones, mu, sigma_ones, sigma_matrix)

        # Return the updated empirical Bayes estimated beta values
    ...
    def WEB_iter(
        ...
        for name in tqdm(self.data_log, disable=False if verbose == 0 else True):
            ...
            for i_1 in range(max_iter):
                ...
                self.weights[name] = calculate_weights(
                    self.Xs_tilde[name], 
                    self.data_log[name], 
                    self.betas_MDPDE[name], 
                    self.sigmas_MDPDE[name], alpha
                    )
                
                self.betas_MDPDE[name] = calculate_betas_MDPDE(
                    self.Xs_tilde[name], 
                    self.data_log[name], 
                    self.weights[name]
                    )
                self.sigmas_MDPDE[name] = calculate_sigmas_MDPDE(
                    self.Xs_tilde[name], 
                    self.data_log[name], 
                    self.betas_MDPDE[name],
                    self.weights[name], alpha)
            ...
            self.mus_MDPDE[name] = calculate_mus_MDPDE(self.betas_MDPDE[name], self.lambdas[name])
            self.sigma_matrices_MDPDE[name] = calculate_sigma_matrix_MDPDE(
                self.betas_MDPDE[name], 
                self.mus_MDPDE[name], 
                self.lambdas[name], 
                0, 
            ...
            for i_2 in range(max_iter):
                # Iterate the algorithm
                self.lambdas[name] = calculate_lamdas(
                    self.betas_MDPDE[name], 
                    gamma, 
                    self.mus_MDPDE[name], 
                    self.sigma_matrices_MDPDE[name]
                    )
                self.mus_MDPDE[name] = calculate_mus_MDPDE(self.betas_MDPDE[name], self.lambdas[name])
                self.sigma_matrices_MDPDE[name] = calculate_sigma_matrix_MDPDE(
                    self.betas_MDPDE[name], 
            ...
            self.mus_mle_wo_lambdas[name] = calculate_mus_mle(
                self.Xs_tilde[name], 
                self.data_log[name], 
                sigma_ones, 
                self.sigma_matrices_MDPDE[name], 
            ...
            self.betas_WEB_wo_lambdas[name] = calculate_betas_WEB(
                self.Xs_tilde[name], 
                self.data_log[name], 
                self.weights[name], 
                lambda_ones, 
                self.mus_mle_wo_lambdas[name], 
                sigma_ones, 
                self.sigma_matrices_MDPDE[name]
                )
            self.mus_mle[name] = calculate_mus_mle(
                self.Xs_tilde[name], 
                self.data_log[name], 
                self.sigmas_MDPDE[name], 
                self.sigma_matrices_MDPDE[name], 
                self.weights[name], 
                self.lambdas[name]
                )
            self.betas_WEB[name] = calculate_betas_WEB(
                self.Xs_tilde[name], 
                self.data_log[name], 
                self.weights[name], 
                self.lambdas[name], 
        ...
        self.densities_WEB = calculate_density(self.distances_to_center, self.betas_WEB, separated=True)
        self.densities_mle = calculate_density(self.distances_to_center, self.mus_mle)
        self.densities_MDPDE = calculate_density(self.distances_to_center, self.mus_MDPDE)
    ...
    def fitted_densities_plot(self, keys, root: Optional[str] = None):
        ...
        fitted_densities = calculate_density(self.distances_to_center, self.betas_WEB, separated=True)
        if keys is not None:
    ...
    def representative_densities_plot(self, 
        ...
        if "WEB Mean-E" in estimators:
            ...
            self.densities_EWEB_Mean = calculate_density(self.distances_to_center, self.betas_EWEB)
        ...
        if "MDPDE Mean-E" in estimators:
            ...
            self.densities_EMDPDE_Mean = calculate_density(self.distances_to_center, self.betas_EMDPDE)
        ...
        if "EB" in estimators:
            self.mus_EB = {name: calculate_mus_mle(
                self.Xs_tilde[name], 
                self.data_log[name], 
                self.sigmas_MDPDE[name], 
                self.sigma_matrices_MDPDE[name],
                np.ones((len(self.Xs_tilde[name]), len(self.distances_to_center[name][0]))), 
                np.ones(len(self.Xs_tilde[name]))
                ) for name in self.data_log}
            self.densities_EB = calculate_density(self.distances_to_center, self.mus_EB)
    ...
    def find_outliers(self, prob: float = 0.95) -> Tuple[Dict[str, List[int]], Dict[str, np.ndarray]]:
        """
        ...
        self.outliers = {}  # Dictionary to store indices of outliers for each amino acid
        self.densities_outliers = {}  # Dictionary to store densities of outliers for each amino acid
        self.statistic_distances0 = {}  # Dictionary to store statistic distances for the first component of each beta
        self.statistic_distances1 = {}  # Dictionary to store statistic distances for the second component of each beta
        self.statistic_distances = {}  # Dictionary to store overall statistic distances for each amino acid
        self.sigmas = {}  # Dictionary to store covariance matrices for each amino acid
        self.margin = chi2.ppf(prob, df=2)  # Threshold for determining outliers based on chi-square distribution

        # Loop through amino acids
        for name in self.betas_WEB:
            self.statistic_distances0[name] = np.array([])  # Initialize array for the first component's statistic distances
            self.statistic_distances1[name] = np.array([])  # Initialize array for the second component's statistic distances
            self.statistic_distances[name] = np.array([])  # Initialize array for overall statistic distances

            mean = self.mus_mle[name]  # Mean values for the amino acid
            betas = self.betas_WEB[name]  # Beta values for the amino acid
            sigma0 = np.sum([(betas - mean)[i][0] ** 2 for i in range(len(betas))]) / len(betas)  # Variance for the first component
            sigma1 = np.sum([(betas - mean)[i][1] ** 2 for i in range(len(betas))]) / len(betas)  # Variance for the second component
            sigma01 = np.sum([(betas - mean)[i][0] * (betas - mean)[i][1] for i in range(len(betas))]) / len(betas)  # Covariance
            self.sigmas[name] = np.array([sigma0, sigma01, sigma01, sigma1]).reshape(2, 2)  # Covariance matrix

            # Loop through beta values
            for i in range(len(betas)):
                # Calculate statistic distances for each component
                statistic_distance0 = (betas - mean)[i][0] ** 2 / sigma0
                statistic_distance1 = (betas - mean)[i][1] ** 2 / sigma1

                # Append the distances to respective arrays
                ...
                # Calculate overall statistic distance using the covariance matrix
                ...
                # Check if the overall statistic distance exceeds the threshold, indicating an outlier
                if (statistic_distance) > self.margin:
                    if name not in self.outliers:
                        self.outliers[name] = []  # Initialize list for outliers
                        self.densities_outliers[name] = []  # Initialize list for outlier densities
                    self.outliers[name].append(i)  # Append the index of the outlier
                    self.densities_outliers[name].append(self.densities_data[name][i])  # Append the density of the outlier

        # Return dictionaries of outliers and statistic distances
